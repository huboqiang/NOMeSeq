...
class NDR(object):
    def __init__(self, chrom, count_u, count_m, tb_file, cutoff = 1e-5, depth = 3, bin_len=40, step_len=20, dist_len=140 ):
        self.chrom    = chrom
...
#                del self.l_rat_umt_met[idx]
                ...
                print "%s\t%d\t%d\t%1.2f\t%d,%d" % (self.chrom, self.bin_begin, self.bin_endin, pval, self.l_umt_met[idx][0], self.l_umt_met[idx][1])
                del self.l_umt_met[idx]
                del self.l_rat_umt_met[idx]
...
#    l_line=  f_cntfile.readlines()
    ...
    m_NDR =  NDR(CHROM, COUNT_U, COUNT_M, tb_file, CUTOFF, DEPTH, bin_len=40, step_len=20, dist_len=140 )
