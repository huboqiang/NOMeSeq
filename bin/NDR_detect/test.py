...
def show_help():
   print >>sys.stderr,      "\n\tpython ",sys.argv[0]," /datd/huboqiang/test_NOM/02.SingleC/mESC_gF28_1/singleC/chr10.ACG.TCG.bed.gz COUNT_U COUNT_M"
...
#        np_idx = np.array([ self.NDR_info['idx'].index(i) for i in l_val ],dtype="int")
        ...
        str_p = ",".join( ["%1.2e" % (pval) for pval in np_pval] )
    ...
    def __chisq_bin(self, idx):
        ...
        if np_obs[0] < np_exp[0]:  
            pval = 1-scipy.stats.chi2.cdf(chisquare, 1)
...
#        print pos, self.bin_idx_min, (self.bin_idx_min) * self.step_len + 1, (self.bin_idx_min) * self.step_len + self.bin_len
#        print pos, self.bin_idx_max, (self.bin_idx_max) * self.step_len + 1, (self.bin_idx_max) * self.step_len + self.bin_len
        if self.bin_idx_min < 0:
            self.bin_idx_min = 0
        
    def __bin_info(self, in_idx):
        """bin to pos"""
        self.bin_begin = (in_idx+1) * self.step_len + 1
